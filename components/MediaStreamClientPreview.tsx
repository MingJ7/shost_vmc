...
import { VMCStreamer, WRTCclientLink } from "./MediapipeSlover";
import { AudioOutputSelector } from "./selectors/output";
import { DataConnection } from "peerjs";

export function ClientPreview ({mediaStream, dataConnection}: {mediaStream: MediaStream | undefined, dataConnection: DataConnection}){
    const [preview, setPreview] = useState(false);
    const [vmcStream, setVMCStream] = useState<VMCStreamer | undefined>()
    const videoRef = useRef(null)
    const [volume, setVolume] = useState(1);
    const [linker, setLinker] = useState(new WRTCclientLink(dataConnection))
